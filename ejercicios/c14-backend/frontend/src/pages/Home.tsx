...
export default function Home() {
    ...
    const destacados = useMemo(() => {
        if (!libros || !destacadosIds) return [];
        return libros.filter((libro) => destacadosIds.includes(libro.id));
    }, [libros, destacadosIds]);
            ...
            <Container className="my-5">
                ...
                {loading ? (
                    ...
                    <Alert variant="danger" className="text-center">
                        Hubo un error al cargar los libros destacados: {error}
                    </Alert>
                ) : destacados.length > 0 ? (
